...
/*   Updated: 2019/01/20 19:30:42 by rcaumett    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static int	select_cleanup(t_select *select)
{
...
static int	select_readline(t_select *select)
{
	...
	while ((readed = read(0, buf, 4)) > 0 && select->status == -1)
	{
		ft_bzero(buf + readed, 4 - readed);
		if (select_action_dispatcher(select, buf))
			return (1);
	}
	return (select_cleanup(select));
}
